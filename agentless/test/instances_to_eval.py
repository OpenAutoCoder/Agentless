...
with jsonlines.open('successful_tests.jsonl', mode='w') as writer:
    for entry in ds["test"]:
        ...
        with open(file_path, 'r') as file:
            data = json.load(file)
        tests_which_pass = data.get(instance_id, {}).get("tests_status", {}).get("PASS_TO_PASS", {}).get("success", [])


        if tests_which_pass == []:
            print(f" {instance_id} didn't get any passing tests for its original {len(tests)}")
