...
def txt_file_contains_string(path_to_txt, expected_output):
    ...
    try:
        with open(path_to_txt, "r", encoding="utf-8") as file:
            content = file.read()
            return expected_output in remove_ansi_sequences(content).replace(
...
def load_existing_results(jsonl_filename):
    ...
    try:
        ...
        print(
            f"No existing results file found at {jsonl_filename}. Proceeding with an empty execution_results dictionary."
        )
    return results
